...
class Dataset:
    ...
    def _get_field_from_config(self):
        ...
        self.bid_field = self.config['BEHAVIOR_ID_FIELD']
        ...
        self.logger.debug(set_color('bid_field', 'blue') + f': {self.bid_field}')
    ...
    def _data_processing(self):
        ...
        """
        self.feat_name_list = self._build_feat_name_list()  # ['inter_feat']
    ...
    def _build_feat_name_list(self):
        ...
        """
        feat_name_list = [
            feat_name for feat_name in ['inter_feat', 'user_feat', 'item_feat', 'behavior_feat']
    ...
    def _load_data(self, token, dataset_path):
        ...
        self.user_feat = self._load_user_or_item_feat(FeatureSource.USER, 'uid_field')
        self.item_feat = self._load_user_or_item_feat(FeatureSource.ITEM, 'iid_field')
        self.behavior_feat = self._load_user_or_item_feat(FeatureSource.BEHAVIOR, 'bid_field')
    ...
    def _load_user_or_item_feat(self, source, field_name):
        ...
        """
        # source: FeatureSource.USER name:'USER' value:'user'
        # feat_path = os.path.join(dataset_path, f'{token}.{source.value}')  # feat_path = 'dataset/ml-1m/ml-1m.user'
        field = getattr(self, field_name, None)  # 'field: user_id'
        x = getattr(self, 'bid_field', None)
        # if
        # if os.path.isfile(feat_path):
        feat = self.inter_feat[[field]]
        feat.columns = [field]
        # self.logger.debug(f'[{source.value}] feature loaded successfully from [{feat_path}].')
        self.logger.debug(f'[{source.value}] feature loaded successfully.')
        # else:
        #     feat = None
        ...
        if field in self.field2source:
            self.field2source[field] = FeatureSource(
                source.value + '_id')  # {'user_id': <FeatureSource.USER_ID: 'inter'>  ==> 'user_id'}
        return feat
    ...
    def _load_feat(self, filepath, source):
        ...
        field_separator = self.config['field_separator']  # config=>final_config_dict
        columns = []
        usecols = []
        dtype = {}
        encoding = self.config['encoding']
        # filepath=>''dataset/ml-1m/ml-1m.inter''
        with open(filepath, 'r', encoding=encoding) as f:
            head = f.readline()[:-1]  # 'user_id:token	seller_id:token	item_id:token	category_id:token	timestamp:float'
        for field_type in head.split(field_separator):
            field, ftype = field_type.split(':')  # field: 'user_id', ftype:'token'
            try:
            ...
            if isinstance(source, FeatureSource) or source != 'link':
                self.field2source[
                    field] = source  # field2source = {dict: 5} {'user_id': <FeatureSource.INTERACTION: 'inter'>, 'seller_id': <FeatureSource.INTERACTION: 'inter'>, 'item_id': <FeatureSource.INTERACTION: 'inter'>, 'category_id': <FeatureSource.INTERACTION: 'inter'>, 'timestamp': <FeatureSource.INTERACTION: 'inter'>}
                self.field2type[
                    field] = ftype  # {'user_id': <FeatureType.TOKEN: 'token'>, 'seller_id': <FeatureType.TOKEN: 'token'>, 'item_id': <FeatureType.TOKEN: 'token'>, 'category_id': <FeatureType.TOKEN: 'token'>, 'timestamp': <FeatureType.FLOAT: 'float'>}
                if not ftype.value.endswith('seq'):
                    self.field2seqlen[field] = 1  # {'user_id': 1, 'item_id': 1, 'rating': 1, 'timestamp': 1}
            columns.append(field)  # ['user_id', 'seller_id', 'item_id', 'category_id', 'timestamp']
            usecols.append(
                field_type)  # ['user_id:token', 'seller_id:token', 'item_id:token', 'category_id:token', 'timestamp:float']
            # {'user_id:token': <class 'str'>, 'seller_id:token': <class 'str'>, 'item_id:token': <class 'str'>, 'category_id:token': <class 'str'>, 'timestamp:float': <class 'numpy.float64'>}
        ...
        df.columns = columns  # ['user_id', 'seller_id', 'item_id', 'category_id', 'timestamp']
    ...
    def _set_alias(self, alias_name, default_value):
        ...
        _, idx = np.unique(alias, return_index=True)  # _:['user_id'] idx: [0]
        self.alias[alias_name] = alias[
            np.sort(idx)]  # {'user_id': array(['user_id'], dtype='<U7'), 'item_id': array(['item_id'], dtype='<U7')}
    ...
    def _init_alias(self):
        ...
        self._set_alias('behavior_id', [self.bid_field])
        ...
        self._rest_fields = self.token_like_fields  # ['user_id', 'item_id']
    ...
    def _remove_duplication(self):
        ...
        if keep is None:
            return
        # self._check_field('uid_field', 'iid_field', 'bid_field')
        ...
        self.inter_feat.drop_duplicates(subset=[self.uid_field, self.iid_field], keep=keep,
                                        inplace=True)
    ...
    def _filter_by_inter_num(self):
        ...
        user_inter_num_interval = self._parse_intervals_str(
            self.config['user_inter_num_interval'])  # user_inter_num_interval: [('(', 4.0, inf, ')')]
        item_inter_num_interval = self._parse_intervals_str(
            self.config['item_inter_num_interval'])  # item_inter_num_interval: [('(', 4.0, inf, ')')]
    ...
    def fields(self, ftype=None, source=None):
        ...
        """
        # fype: [<FeatureType.TOKEN: 'token'>, <FeatureType.TOKEN_SEQ: 'token_seq'>]
        # source: {<FeatureSource.INTERACTION: 'inter'>, <FeatureSource.USER: 'user'>, <FeatureSource.NET: 'net'>, <FeatureSource.ITEM: 'item'>, <FeatureSource.USER_ID: 'user_id'>, <FeatureSource.ITEM_ID: 'item_id'>, <FeatureSource.KG: 'kg'>}
        ftype = set(ftype) if ftype is not None else set(
            FeatureType)  # {<FeatureType.TOKEN: 'token'>, <FeatureType.TOKEN_SEQ: 'token_seq'>}
        source = set(source) if source is not None else set(FeatureSource)  # set: 7
        ret = []
    ...
    def field2feats(self, field):
        ...
        elif field == self.bid_field:
            feats = [self.inter_feat]
            if self.behavior_feat is not None:
                feats.append(self.behavior_feat)
        else:
    ...
    @property
    def behavior_counter(self):
        """Get the counter containing the occurrences times in ``inter_feat`` of different items.

        Returns:
            Counter: The counter of different items.
        """
        self._check_field('bid_field')
        return self.counter(self.bid_field)
    ...
    def __str__(self):
        ...
        if self.bid_field:
            info.extend([
                set_color('The number of purchase', 'blue') + f': {self.behavior_counter.get(2)}',
                set_color('The number of click', 'blue') + f': {self.behavior_counter.get(1)}'
                # set_color('Average actions of items', 'blue') + f': {self.avg_actions_of_items}'
            ])
