...
def create_dataset(config):
    ...
    if hasattr(dataset_module, config['model'] + 'Dataset'):
        ...
        model_type = config['MODEL_TYPE']  # 'SEQUENTIAL'
        type2class = {
    ...
    # default_file==》'saved/ml-1m-SequentialDataset.pth'
    default_file = os.path.join(config['checkpoint_dir'], f'{config["dataset"]}-{dataset_class.__name__}.pth')
    file = config['dataset_save_path'] or default_file   # file==>'saved/ml-1m-SequentialDataset.pth'
