...
class Dataset:
    ...
    def _load_user_or_item_feat(self, token, dataset_path, source, field_name):
        ...
        if field in self.field2source:
            self.field2source[field] = FeatureSource(source.value + '_id')  # {'user_id': <FeatureSource.USER_ID: 'inter'>  ==> 'user_id'}
        return feat
    ...
    def _load_feat(self, filepath, source):
        ...
        for field_type in head.split(field_separator):
            ...
            if isinstance(source, FeatureSource) or source != 'link':
                self.field2source[field] = source  # field2source = {dict: 5} {'user_id': <FeatureSource.INTERACTION: 'inter'>, 'seller_id': <FeatureSource.INTERACTION: 'inter'>, 'item_id': <FeatureSource.INTERACTION: 'inter'>, 'category_id': <FeatureSource.INTERACTION: 'inter'>, 'timestamp': <FeatureSource.INTERACTION: 'inter'>}
                self.field2type[field] = ftype  # {'user_id': <FeatureType.TOKEN: 'token'>, 'seller_id': <FeatureType.TOKEN: 'token'>, 'item_id': <FeatureType.TOKEN: 'token'>, 'category_id': <FeatureType.TOKEN: 'token'>, 'timestamp': <FeatureType.FLOAT: 'float'>}
            ...
            usecols.append(field_type)  # ['user_id:token', 'seller_id:token', 'item_id:token', 'category_id:token', 'timestamp:float']
    ...
    def _set_alias(self, alias_name, default_value):
        ...
        self.alias[alias_name] = alias[np.sort(idx)]  # {'user_id': array(['user_id'], dtype='<U7'), 'item_id': array(['item_id'], dtype='<U7')}
    ...
    def _filter_by_inter_num(self):
        ...
        user_inter_num_interval = self._parse_intervals_str(self.config['user_inter_num_interval'])  # user_inter_num_interval: [('(', 4.0, inf, ')')]
        item_inter_num_interval = self._parse_intervals_str(self.config['item_inter_num_interval'])  # item_inter_num_interval: [('(', 4.0, inf, ')')]
    ...
    def fields(self, ftype=None, source=None):
        ...
        ftype = set(ftype) if ftype is not None else set(FeatureType)  # {<FeatureType.TOKEN: 'token'>, <FeatureType.TOKEN_SEQ: 'token_seq'>}
