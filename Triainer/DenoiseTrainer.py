...
class DenoiseTrainer(Trainer):
    ...
    def __init__(self, config, model, Graph):
        ...
        self.graph = Graph.getSparseGraph()
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False):
        ...
        """
        self.model.train()
        if 'Denoise' not in str(self.model):
            loss_func = self.model.calculate_loss
        else:
            loss_func = self.model.calculate_loss_denoise
        # loss_func = self.model.calculate_loss_denoise
        ...
        for batch_idx, interaction in enumerate(iter_data):
            ...
            # 每100个batch退火一次
            if self.global_train_batches % 4000 == 0 and self.gumbel_tau_anneal:
            ...
            losses, generated_seq_loss, loss_filter_loss, clean_seq_percent = loss_func(interaction,
                                                                             self.graph,
                                                                             self.drop_rate_schedule_curriculum(epoch_idx),
                                                                             self.tau)  # reconstruction_loss [batch_size]
            ...
            if self.gpu_available and show_progress:
                iter_data.set_postfix_str(set_color('gumbel tau: ' + '%.2f' % self.tau, 'yellow') + ', ' +
                                          ...
                                          set_color('推荐损失: ',
                                                    'blue') + '%.6f' % generated_seq_loss.data.__float__() + ', ' +
        ...
        if self.scheduler:
            ...
            self.scheduler.step()
        return total_loss, total_percent / len(train_data), total_gen
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None):
        ...
        for epoch_idx in range(self.start_epoch, self.epochs):
            ...
            train_loss, clean_item_percent, total_gen = self._train_epoch(train_data, epoch_idx,
